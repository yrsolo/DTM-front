...
- `UI registry` в админке фиксирует повторяющиеся UI-элементы как отдельный inventory layer, но в V1 не меняет runtime styling напрямую.
...
- `Пресеты` и `UI registry` выполняют разные роли: presets управляют color/layout runtime-настройкой, а UI registry описывает повторяющиеся визуальные primitives для дальнейшей унификации.
