...
- `UI registry` в админке фиксирует реальные повторяющиеся UI-элементы проекта как отдельный inventory layer, но не меняет runtime styling напрямую.
...
- `Пресеты` и `UI registry` выполняют разные роли: presets управляют color/layout runtime-настройкой, а UI registry собирает живой визуальный аудит реальных элементов со всех поверхностей проекта для дальнейшей унификации.
