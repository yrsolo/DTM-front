...
- hero с крупным headline, screenshot-led product scene и ручным запуском видео как secondary media;
- центральная колонка смысловых блоков, в которой ключевые секции опираются на реальные product screenshots;
- боковые декоративные silhouette-slots под будущих персонажей;
- финальный CTA, ведущий в обычный вход DTM.
...
- hero и product-секции используют реальные viewer-safe кадры DTM:
  ...
  - task drawer;
  - mobile view;
- каждый смысловой блок по умолчанию показывает короткую версию текста;
- по клику `Подробнее` короткий текст заменяется на подробный прямо в том же блоке;
